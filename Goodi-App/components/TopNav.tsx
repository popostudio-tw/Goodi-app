...
const NAV_ITEMS = [
    ...
    { page: Page.Tree, label: '樹洞', icon: 'https://api.iconify.design/twemoji/deciduous-tree.svg', requiredPlan: 'paid499', color: 'bg-teal-400 hover:bg-teal-500', lockedColor: 'bg-gray-200' },
    { page: Page.FocusTimer, label: '番茄鐘', icon: 'https://api.iconify.design/twemoji/timer-clock.svg', requiredPlan: 'paid199', color: 'bg-orange-400 hover:bg-orange-500', lockedColor: 'bg-gray-200' },
    { page: Page.Achievements, label: '成就', icon: 'https://api.iconify.design/twemoji/trophy.svg', requiredPlan: 'paid499', color: 'bg-purple-400 hover:bg-purple-500', lockedColor: 'bg-gray-200' },
  ...
  return (
    <div className="grid grid-cols-4 md:grid-cols-8 gap-2 sm:gap-3">
        {NAV_ITEMS.map((item) => {
            ...
            // Lock logic
            let isLocked = false;
            if (item.requiredPlan === 'paid499' && plan !== 'paid499') isLocked = true;
            if (item.requiredPlan === 'paid199' && plan === 'free') isLocked = true;
