...
const OnboardingModal: React.FC = () => {
    const { updateUserData, addToast, handleApplyReferralCode } = useUserData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (referralCode.trim()) {
                ...
                // Success toast is handled by handleApplyReferralCode
            }
        } catch (error) {
            ...
            <div className="mt-8 sm:mt-12 bg-white/60 backdrop-blur-xl border border-white/50 rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center animate-fade-in transform transition-transform">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        ...
                        className="w-full bg-blue-600/90 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg ring-2 ring-white/30 backdrop-blur-sm mt-4"
                    >
                        {isSubmitting ? '設定中...' : '歡迎一起進入 Goodi 學園!'}
                    </button>
                </form>
            </div>
        </div>
