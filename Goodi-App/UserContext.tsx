...
export const commonTasksData: Omit<
  Task,
  "id" | "completed" | "isHabit" | "consecutiveCompletions" | "isSpecial" | "dateRange" | "consecutiveCompletions"
>[] = [
...
export const UserDataProvider: React.FC<UserDataProviderProps> = ({ children, user, addToast }) => {
  const [userData, setUserData] = useState<Omit<UserData, "lastLoginDate">>(() => {
    ...
    if (savedData) {
      try {
        ...
        if (parsed.shopRewards) parsed.shopRewards = parsed.shopRewards.filter((r: Reward) => r.id !== 304);
        return parsed;
      } catch (e) {
    ...
    if (lastCheckDate !== todayStr) {
      ...
      const resetTasks = userData.tasks.map((task) => {
        ...
        if (task.isHabit || task.category === "生活" || task.category === "家務") {
          if (!task.completed && !wasYesterdayFrozen && !task.mastered) newConsecutive = 0;
        }
        return { ...task, completed: false, consecutiveCompletions: newConsecutive };
      });

      setUserData((prev) => ({ ...prev, tasks: resetTasks }));
  ...
  const saveData = useCallback(
    async (dataToSave: Omit<UserData, "lastLoginDate">) => {
      if (!user) return;
      try {
  ...
  const updateUserData = useCallback(
    (updates: Partial<Omit<UserData, "lastLoginDate">>) => {
  ...
  const unlockAchievement = useCallback(
    (id: string, customTitle?: string, customIcon?: string, videoId?: string) => {
      ...
      if (existingAch) {
        ...
        updateUserData({ inventory: [newItem, ...userData.inventory] });
  ...
  const handleCompleteTask = (taskId: number, isProactive: boolean) => {
    ...
    if (task.mastered) pointsGained = Math.floor(pointsGained * 1.5);

    const newTransaction: Transaction = {
      id: Date.now(),
      description: `完成任務: ${task.text} ${isProactive ? "(主動)" : ""} ${task.mastered ? "(大師加成)" : ""}`,
      amount: `+${pointsGained} 積分`,
      timestamp: Date.now(),
    };
    ...
    const newTasks = userData.tasks.map((t) => {
      ...
      const updatedTask: any = { ...t, completed: true };
      ...
      if (updatedTask.isHabit || updatedTask.category === "生活" || updatedTask.category === "家務") {
        ...
        if (updatedTask.consecutiveCompletions >= 21 && !updatedTask.mastered) {
          ...
          setTimeout(() => {
            unlockAchievement(`mastery_${updatedTask.id}`, `${updatedTask.text}大師`, updatedTask.icon, "https://video.wixstatic.com/video/ec806c_0aad9d677de244edbf8c44d351133f58/720p/mp4/file.mp4");
          }, 500);
        }
      }
      return updatedTask;
    ...
    const praiseMsg = task.mastered ? `大師級表現！獲得 ${pointsGained} 積分 (含加成)！` : `任務完成！獲得 ${pointsGained} 積分！`;
  ...
  const handlePraiseSubmit = (taskId: number, isProactive: boolean, praiseText: string) => {
    ...
    const newTransaction: Transaction = { id: Date.now(), description: `完成任務: ${task.text} ${isProactive ? "(主動)" : ""}`, amount: `+${pointsGained} 積分`, timestamp: Date.now() };
  ...
  const handleExchange = (pointsToSpend: number, tokensToGet: number) => {
    if (userData.points < pointsToSpend) {
      addToast("積分不足！");
      return false;
    }
  ...
  const handleBuyReward = (reward: Reward) => {
    if (userData.tokens < reward.cost) {
      addToast("代幣不足！");
      return false;
    }
  ...
  const handleMakeWish = (wish: string): boolean => {
    ...
    updateUserData({ tokens: userData.tokens - cost, wishes: [wish, ...userData.wishes] });
  ...
  const handleUseItem = (id: number, callbacks: { onStartParentChildTime: () => void }) => {
    ...
    if (item.action === "parent_child_time" && item.durationMinutes) {
      callbacks.onStartParentChildTime();
      return;
    }
  ...
  // ✅ 这里是关键：不在前端用 Gemini Key，改成 callable functions
  const handleAddEntry = async (text: string) => {
    ...
    // 1) 安全检查（Functions）
    try {
      const safetyCheck = httpsCallable(functions, "safetyCheckJournalEntry");
      const res = await safetyCheck({ text });
      const flag = (res.data as any)?.flag === true;
      if (flag) {
        updateUserData({
      ...
      console.error("safetyCheckJournalEntry failed:", e);
      // 安全检查失败不挡流程
    }

    // 2) 生成 Goodi 回复（Functions）
    try {
      const replyFn = httpsCallable(functions, "generateJournalReply");
      const res = await replyFn({ text });
      const replyText = ((res.data as any)?.text || "").trim();
      const goodiText = replyText || "嗚，Goodi 的訊號好像不太好，等一下再試一次好嗎？";
      const goodiEntry: JournalEntry = { id: Date.now() + 1, text: goodiText, date: new Date().toISOString(), author: "goodi" };

      updateUserData({ journalEntries: [...userData.journalEntries, userEntry, goodiEntry] });
    } catch (e) {
      console.error("generateJournalReply failed:", e);
  ...
  const handleChildAddTask = (text: string, frequency: "today" | "everyday" | "schooldays") => {
    ...
    if (frequency === "everyday") schedule = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"];
    else if (frequency === "schooldays") schedule = ["mon", "tue", "wed", "thu", "fri"];
    else if (frequency === "today") dateRange = { start: todayStr, end: todayStr };
  ...
  const handleFocusSessionComplete = (durationInSeconds: number) => {
    ...
    const newCounts = { ...userData.focusSessionCounts, [durationInMins]: (userData.focusSessionCounts as any)[durationInMins] || 0 + 1 } as any;
    gainPoints(2);
  ...
  const handleShareMessage = (message: string) => {
    ...
    updateUserData({ sharedMessages: [message.trim(), ...userData.sharedMessages] });
    gainPoints(5);
  ...
  const handleManualPointAdjustment = useCallback(
  ...
  const handleAddKeyEvent = useCallback(
    (text: string, date: string) => {
      if (!text.trim()) return;
      const newEvent: KeyEvent = { id: Date.now(), date, text: text.trim() };
