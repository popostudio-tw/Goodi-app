...
const FeaturedTaskListItem = React.memo(({ task, onCompleteTask, onReportPraise, tag }: { task: Task; onCompleteTask: (taskId: number, isProactive: boolean) => void; onReportPraise: (info: { taskId: number, isProactive: boolean }) => void; tag: string; }) => {
...
const DailyTaskCard = React.memo(({ task, onCompleteTask, onReportPraise }: { task: Task; onCompleteTask: (taskId: number, isProactive: boolean) => void; onReportPraise: (info: { taskId: number, isProactive: boolean }) => void; }) => {
    const isPraiseTask = task.id === 27;
    const handleComplete = (isProactive: boolean) => {
        if (isPraiseTask) onReportPraise({ taskId: task.id, isProactive }); else onCompleteTask(task.id, isProactive);
    };
