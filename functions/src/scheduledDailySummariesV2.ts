...
/**
 * 核心生成邏輯：為單一用戶生成豐富的昨日總結
 * @param db Firestore 實例
 * @param userId 用戶 ID
 * @param yesterdayDate 昨日日期 YYYY-MM-DD
 * @param userData 用戶資料 (userDoc.data())
 * @returns 生成的總結資料物件
 */
export async function generateRichDailySummary(
    db: Firestore,
    userId: string,
...
**僅輸出總結文字，不要標題或分號或其他格式。**`;
    ...
    // === 8. 儲存總結到 Firestore ===
    await db.doc(`users/${userId}/dailySummaries/${yesterdayDate}`).set(resultData);
        ...
        try {
            ...
            for (const userDoc of usersSnapshot.docs) {
                ...
                try {
                    ...
                    // 調用 Helper
                    const result = await generateRichDailySummary(db, userId, yesterdayDate, userData);

                    if (result) {
                        processedCount++;
                        console.log(`[DailySummary] ✓ Generated summary for ${userId}`);
                    } else {
                        skippedCount++; // No tasks
                    }
